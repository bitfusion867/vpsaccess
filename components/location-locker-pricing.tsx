...
import { locationLockerPlans } from "@/lib/data"


export function LocationLockerPricingSection() {
    const [currentSlide, setCurrentSlide] = useState(0)
    const isMobile = useIsMobile()

    const nextSlide = () => {
        setCurrentSlide((prev) => (prev + 1) % locationLockerPlans.length)
    }

    const prevSlide = () => {
        setCurrentSlide((prev) => (prev - 1 + locationLockerPlans.length) % locationLockerPlans.length)
    }
        ...
        <section id="pricing" className="py-20 px-4 sm:px-6 lg:px-8 bg-muted/30">
            <div className="container mx-auto">
                <div className="text-center mb-16">
                    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-4">Choose A Plan To Access Your Location Locker</h2>
                    ...
                    <div className="md:hidden">
                        ...
                        <div className="flex justify-center items-center mt-6 space-x-4">
                            ...
                            <div className="flex space-x-2">
                                {locationLockerPlans.map((_, index) => (
                                    <button
                                        key={index}
...
function PricingCard({ plan, mobile = false }: { plan: (typeof locationLockerPlans)[0]; mobile?: boolean }) {
    return (
        <Card
        ...
        >
            ...
            <CardFooter className={mobile ? "pt-4" : "pt-6"}>
                <Button
                    ...
                    asChild
                >
                    <Link href={`/checkoutv2?plan=${plan._name.toLowerCase()}`}>Choose Plan</Link>
                </Button>
            </CardFooter>
        </Card>
    )
